...
<script>
var target_goal = "Retirement";
var goal_name = "Retirement";
var current_age = 30;
var retire_age = 60;
var life_age = 80;
var amount = 50000;
var savings_amount = 100000;
var inflation = 5;
var corpus_return = 8;
var age = 30;
var returns = 8;
...
$(document).ready(function(){
	...
	if(currentAge != "")
	{
		...
		$("#txt_current_age").val(current_age);
		$("#txt_retire_age").val(retire_age);
		$("#txt_life_age").val(life_age);
		$("#txt_amount").val(amount);
		$("#txt_savings_amount").val(savings_amount);
		$("#txt_inflation").val(inflation);
		$("#txt_return").val(corpus_return);
		$("#sel_risk").val(risk);
	...
	$("#retire_slider").slider({
		...
		$("#txt_retire_age").val(slideEvt.value);
	});
	...
	$("#life_slider").slider({
		...
		$("#txt_life_age").val(slideEvt.value);
	});
	...
	$("#return_slider").slider({
		...
		$("#txt_return").val(slideEvt.value);
	});

	$("#savings_slider").slider({
		min: 1,
	    max: 10000000,
	    tooltip: "show",
	    step: 1000,
		...
		$("#txt_savings_amount").val(slideEvt.value);
	});
...
{
	var retireAge = $('#txt_retire_age').val();
	$( "#retire_slider" ).slider("setValue",retireAge);
}
function lifeAgeChange()
{
	var lifeAge = $('#txt_life_age').val();
	$( "#life_slider" ).slider("setValue",lifeAge);
}
...
{
	var returns = $('#txt_return').val();
	$( "#return_slider" ).slider("setValue",returns);
}
function savingsAmountChange()
{
	var savings_amount = $('#txt_savings_amount').val();
	$( "#savings_slider" ).slider("setValue",savings_amount);
}
function goToStep1()
{
	$(".robo1").show();
...
{
	current_age = $("#txt_current_age").val();
	retire_age = $("#txt_retire_age").val();
	life_age = $("#txt_life_age").val();
	amount = $("#txt_amount").val();
	savings_amount = $("#txt_savings_amount").val();
	inflation = $("#txt_inflation").val();
	corpus_return = $("#txt_return").val();
	risk = $("#sel_risk").val();
    ...
    if(current_age == "" || isNaN(current_age))
 	{
 		swal({title:"",text:"Please enter your current age"});
		return;
 	}
    if(parseInt(current_age) < 18)
 	{
 		swal({title:"",text:"Minors (under the age of 18) cannot invest without a guardian! Only the legal guardian is allowed to invest on behalf of Minor!"});
		return;
 	}
    if(retire_age == "" || isNaN(retire_age))
 	{
 		swal({title:"",text:"Please enter your retirement age"});
		return;
 	}
    if(life_age == "" || isNaN(life_age))
 	{
 		swal({title:"",text:"Please enter your life expectancy"});
		return;
 	}
    if(amount == "" || isNaN(amount) || parseInt(amount) < 1)
 	{
 		swal({title:"",text:"Please enter your current monthly household expenses"});
		return;
 	}
    if(savings_amount == "" || isNaN(savings_amount) || parseInt(savings_amount) < 1)
 	{
    	savings_amount = 0;
 	}
    if(inflation == "" || isNaN(inflation))
 	{
 		swal({title:"",text:"Please enter expected inflation rate over the years"});
		return;
 	}
    if(corpus_return == "" || isNaN(corpus_return))
 	{
 		swal({title:"",text:"Please enter corpus return after retirement"});
		return;
 	}
    if(risk == "")
 	{
 		swal({title:"",text:"Please select your risk profile"});
		return;
 	}
    if(goal_name == "")
 	{
    ...
    life_age = parseInt(life_age);
    amount = parseInt(amount);
    savings_amount = parseInt(savings_amount);
    inflation = parseInt(inflation);
    corpus_return = parseInt(corpus_return);
    
    horizon = retire_age - current_age;
    if(horizon <= 0)
 	{
 		swal({title:"",text:"Please enter retirement age greater than current age."});
		return;
 	}
    var years = life_age - retire_age;
    if(years <= 0)
 	{
 		swal({title:"",text:"Please enter life expectancy greater than retirement age."});
		return;
 	}
    
    age = current_age;
    
    var curentAge = parseInt(current_age);
	var retireAge = parseInt(retire_age);
	var lifeAge = parseInt(life_age);
	var infRate = parseInt(inflation);
	...
	var exInflation = infRate/100;
	var yrtr = retireAge - curentAge;
	var pv = amount;
	
	var future_value = pv * Math.pow(1 + exInflation, yrtr);
	future_value = Math.round(future_value);
	//alert("future_value--->"+future_value);
	
	var retInvest1 = parseInt(corpus_return) / 100;
	var retInvest2 = parseInt(infRate) / 100;
	var interest_rate  = ((1+retInvest1)/(1+retInvest2)-1) * 0.083333;
	//var retInvest1 = parseInt(corpus_return) / 100;
	//var interest_rate  = (retInvest1/12);
	var number_of_months = (lifeAge - retireAge) * 12;
	var total_amount = 0;
	//alert(interest_rate);
	
	if(interest_rate == 0)
	{
		total_amount = parseFloat(future_value * number_of_months);
		total_amount = Math.round(total_amount);
		//alert("total_amount--->"+total_amount);
	}else
	{
		total_amount = parseFloat(future_value / interest_rate * (1 - Math.pow(1 + interest_rate, -number_of_months)));
		total_amount = Math.round(total_amount);
		//alert("total_amount--->"+total_amount);
	}
	
	//Find Savings amount future value
	var value1 = 1 + (retInvest / 100);
	var value2 = Math.pow(value1, horizon);
	var FV = savings_amount * value2;
    
	var FV1 = Math.ceil(FV);
	future_value_amount = Math.round(FV1);
	//alert("future_value_amount--->"+future_value_amount);
	total_amount = total_amount - future_value_amount;
	if(total_amount < 0)
	{
		total_amount = 0;
	}
	...
	target_amount = parseFloat(total_amount);
	//alert("target_amount--->"+target_amount);
	
	var value3 = 1 + ((retInvest / 100) / 12);
	var months = yrtr * 12;
	...
	var monthly_savings = Math.round(target_amount / value4);
	sip_amount = Math.ceil(monthly_savings / 100.0) * 100.0;
	//alert("sip_amount--->"+sip_amount);
	
	$("#span_invest_today_cost").html(numbersWithComma(amount));
	$("#span_invest_future_cost").html(numbersWithComma(target_amount));
	$("#span_sch_future_cost").html(numbersWithComma(target_amount));
	$("#span_current_savings").html(numbersWithComma(savings_amount));
	$("#span_future_savings").html(numbersWithComma(future_value_amount));
	$("#span_no_years").html(horizon);
	$("#span_sip_amt").html(numbersWithComma(sip_amount));
	$(".span_inflation").html(inflation);
	...
	$("#txt_future_savings_amount").val(numbersWithComma(future_value_amount));
	$(".span_inflation").html(inflation);
...
function checkLoginDetails()
{
	...
	$.post("/validateLogin", {mobile : ""+mobile+"", password : ""+password+""}, function(data)
    {
		...
		function(){
			top.location = "/goal/retirement?current_age="+current_age+"&retire_age="+retire_age+"&life_age="+life_age+"&expense_amount="+amount+"&savings_amount="+savings_amount+"&inflation="+inflation+"&corpus_return="+corpus_return+"&risk="+risk+"&goal_name="+goal_name;
		});
		return;
...
<div class="title-v1">
	<h1 class="goal_name text-center">Retirement Goal</h1>
</div>
</div>
</div>
...
<div class="col lamount" style="float:none;">
	<div><b>Retirement age</b></div>
	<input type="text" value="60" id="txt_retire_age" class="fc-2" onchange="retireAgeChange()" maxlength="2" />
	<div id="retire_slider" style="margin:0px;"></div>
</div>

<div class="col lamount" style="float:none;">
	<div><b>Life expectancy</b></div>
	<input type="text" value="80" id="txt_life_age" class="fc-2" onchange="lifeAgeChange()" maxlength="2" />
	<div id="life_slider" style="margin:0px;"></div>
</div>
</div>
</div>
</div>
...
<div class="col lamount" style="float:none;">
	<div><b>Retirement corpus Return </b></div>
	<input type="text" value="8" id="txt_return" class="fc-2" onchange="returnChange()" maxlength="2" />
	<div id="return_slider" style="margin:0px;"></div>
</div>
</div>
</div>
</div>
...
<div class="row" style="width: 100%;margin:0px;">
<div class="col lamount" style="float:none;">
	<div><b>Savings amount</b></div>
	<input type="text" value="100000" id="txt_savings_amount" class="fc-2" onchange="savingsAmountChange()" maxlength="12" />
	<div id="savings_slider" style="margin:0px;"></div>
</div>
...
<button class="btn btn-primary hidden-xs" style="margin:0 auto; float: none;" onclick="checkResult()">Build My Retirement Goal</button>
</div>
</div>
...
<div class="row"> 
<div class="col-md-12">
<h1 class="text-center">Retirement Goal</h1>
</div>
</div>
</div>
...
<div class="row">
	<div class="col-lg-6 col-sm-4 text-center mb-3">
		<div class="box bg-tf p-3">
			...
			<p>Your Current Monthly household Expenses<br>(in today's value)</p>
		</div>
	</div>
	<div class="col-lg-6 col-sm-4 text-center mb-3">
		<div class="box bg-tf p-3">
			<h6><i class="fa fa-inr" aria-hidden="true"></i> <span id="span_invest_future_cost"></span></h6>
			<p>Retirement Corpus Amount<br>(adjusting for <span class="span_inflation"></span>% inflation)</p>
		</div>
	</div>
	<div class="col-lg-6 col-sm-4 text-center mb-3">
		<div class="box bg-tf p-3">
			<h6><span id="span_current_savings"></span></h6>
			<p>Your Current Savings Now</p>
			<p></p>
		</div>
	</div>
	<div class="col-lg-6 col-sm-4 text-center mb-3">
		<div class="box bg-tf p-3">
			<h6><span id="span_future_savings"></span></h6>
			<p>Future Value of your current Savings</p>
			<p></p>
		</div>
	</div>
	<div class="col-lg-6 col-sm-4 text-center mb-3">
	...
	<div class="col-lg-6 col-sm-4 text-center mb-3">
		<div class="box bg-tf p-3">
			<h6><b><i class="fa fa-inr" aria-hidden="true"></i> <span id="span_sip_amt"></span></b></h6>
...
<c:otherwise>
	<p>If you have an existing portfolio, please click <a href="https://mfportfolio.dakshincapital.com">LOGIN</a> to map investments with your Goal.</p>
</c:otherwise>
</c:choose>
</div>
</div>
...
<tbody>
<tr>
<td class="border-top-0">Retirement Corpus Amount (Inflation adjusted <span class="span_inflation">5</span>% per annum)</td>
<td class="border-top-0"> <input type="text" class="fc-2 fundNamebg" value="" id="txt_goal_amount" disabled="disabled"></td>
</tr>
<tr>
<td class="border-top-0">Future Value of your current Savings</td>
<td class="border-top-0"> <input type="text" class="fc-2 fundNamebg" value="" id="txt_future_savings_amount" disabled="disabled"></td>
</tr>
<tr>
