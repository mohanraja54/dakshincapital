...
function getData()
{
	...
	{
		$("#adv-alert-msg").html("Please select amount");
   	    $("#adv-alert").modal('show');
		return;
	}
 	
	top.location =  "/mutual-funds-research/top-performing-sip-plans-elss-funds?period="+period+"&amount="+amount;
}
...
function downloadXl()
{
	...
	//top.location =  "/mutual-funds-research/downloadTopPerformingSIPELSSFundsXl?period="+period+"&amount="+amount;
	
	var path = "/mutual-funds-research/downloadTopPerformingSIPELSSFundsXl?period="+period+"&amount="+amount;
	    ...
	    if (xhr.readyState === xhr.DONE) {
	        if (xhr.status === 200) {
	        	...
	        	if(contentType == "application/json;charset=ISO-8859-1"){
	    	    	...
	    	    	top.location =  "/mutual-funds-research/downloadTopPerformingSIPELSSFundsXl?period="+period+"&amount="+amount+"&download=yes";
	    	    }
	        }
	    }
	};
...
function openLogin(){
	...
	downloadPath = "/mutual-funds-research/downloadTopPerformingSIPELSSFundsXl?period="+period+"&amount="+amount;
	imageDownload = "no";
	chartId = "";
	filename = "";
...
<section class="main-container mf-research">
	...
	<div class="row ml-0 mr-0">
		<div class="col-md-3 bg-mfbar py-3">
			<h1 class="amfi-title calc-title bold mb-0 pl-3 pt-4">Top Performing SIP ELSS Funds</h1>
		</div>
		<div class="col-md-9 bg-mfbar py-3">
        	...
        	<div class="col-md-2 col-sm-3">
	         ...
	         <div class="col-md-2 col-sm-2">
	         	   <a href="javascript:void(0)" class="btn btn-primary btn-xss" style="position: absolute;top: 35%;width: 70%;" onclick="getData()">Submit</a>	         	   
	         </div>
		</div>
	</div>
	</div>
		        ...
		        <c:choose>
				  	...
				  	<c:otherwise>
			  			<a onClick="downloadXl();" style="cursor:pointer;float:right;color:white;text-decoration:none;" class="btn btn-sm btn-success">Download Result</a>	
							...
							<div class="col-md-12 col-sm-12 marginTop20 table-responsive">
						       <table class="adv-table table style="width:100%" id="tbl_sip_scheme_returns">
						        <thead>
						        		...
						        		<c:choose>
											<c:when test="${fn:contains(header['User-Agent'],'Mobile')}">
											    <tr>
									    	        ...
									    	        <th data-priority="2" class="text-right">Invested Amount</th>
									    	        <th data-priority="3" class="text-right">Current Value</th>
									    	        <th data-priority="4" class="text-right">Return&nbsp;(%)</th>						
									             </tr>		  
											</c:when>
											<c:otherwise>
												 <tr>
						            ...
						            <tbody>
						             ...
						             <tr>
						                <td><a href='/mutual-funds-research/<c:out value="${schemePerformances.scheme_amfi_url}"></c:out>' class="mutual-funds-anchor"><c:out value="${schemePerformances.scheme_name}"></c:out></a>
						                </td>
